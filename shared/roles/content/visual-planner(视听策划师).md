...
- **黄金 3-5 秒法则（Gold 3-5s Hook Rule，开头分镜硬性约束）**：开场第一个分镜（如 `@IntroScene`）必须在前 3-5 秒内通过**“极速视觉切换（如：改配置 1 秒出片结果先行 / 实操痛苦对比）”**来瞬间锁住用户注意力。严禁在开头 15 秒内使用单一、缓慢渐显的静态标题或无实质内容的粒子背景。前 5 秒内必须有高冲击性的实质画面发生。
